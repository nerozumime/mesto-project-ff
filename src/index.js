...
const profileEditInputName = popupContent.querySelector(
  ".popup__input_type_name"
);
const profileEditInputDescription = popupContent.querySelector(
...
const newPlaceInputTitle = popupAddNewPlace.querySelector(
  ".popup__input_type_card-name"
);
const newPlaceInputLink = popupAddNewPlace.querySelector(
...
function handleAddNewPlaceSubmit(evt) {
  ...
  getAddNewCard(newPlaceInputTitle.value, newPlaceInputLink.value)
    .then((card) => {
      placesList.prepend(
        addCard(card, profileId, tryDeleteCard, likeCard, showFullImage)
      );
...
Promise.all([getProfileData(), getInitialCardsData()])
  .then(([profileData, cardsData]) => {
    ...
    cardsData.forEach((card) => {
      placesList.append(
        addCard(card, profileData._id, tryDeleteCard, likeCard, showFullImage)
      );
    });
  })
  .catch((err) => {
...
const popupInputChangeAvatar = formChangeAvatar.querySelector(
...
function likeCard(card, cardId) {
  ...
  if (
    ...
    deleteLike(cardId).then((card) => {
      card.likes.length > 0
        ? likeCounter.setAttribute("style", "opacity: 1;")
        : likeCounter.setAttribute("style", "opacity: 0;");
